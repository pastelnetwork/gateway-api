...
def test_create_cascade_job(db: Session) -> None:
    ...
    assert burn_tx.result_id == result_id

    # Get bound by result_id
    burn_tx_from_db = crud.preburn_tx.get_bound_to_result(db, result_id=result_id)
    assert burn_tx_from_db.fee == 11111
    assert burn_tx_from_db.height == 1000000
    assert burn_tx_from_db.txid == burn_txid
    assert burn_tx_from_db.result_id == result_id
    ...
    assert free_burn_tx.result_id == result_id
