        ...
        if task_from_db.wn_task_id:
            ...
            if not wn_task_status:
                ...
                if height - task_from_db.height > 48:
                    ...
                    with db_context() as session:
                        mark_task_in_db_as_failed(session, task_from_db, update_task_in_db_func,
                                                  get_by_preburn_txid_func)
                continue
                ...
                if not task_from_db.act_ticket_txid:
                    if task_from_db.reg_ticket_txid:
                        ...
                        if act_ticket and 'txid' in act_ticket and act_ticket['txid']:
                            ...
                            finalize_registration(task_from_db, act_ticket['txid'], update_task_in_db_func)
                            break
                    act = status.split(f'Activated {service_name} Action Ticket TXID: ', 1)
                    if len(act) == 2:
                        logger.info(f"Found act ticket txid from WalletNode: {act[2]}")
                        finalize_registration(task_from_db, act[2], update_task_in_db_func)
                        break


def finalize_registration(task_from_db, act_txid, update_task_in_db_func):
...
def clear_task_in_db(task_from_db, update_task_in_db_func):
